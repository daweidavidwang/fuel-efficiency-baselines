...
pcc_fmr = FM(np.mean(fmr.vehicle_mass_kg))
...
for ai in range(len(acc_arange)):
    for vi in range(len(v_arange)):
        for si in range(len(slope_arange)):
            ...
            if not math.isnan(result[ai, vi, si]):
                error_between_pcc.extend([(result[ai, vi, si] - pcc_fmr.cal_fuel_rate(acc_arange[ai], v_arange[vi], slope_arange[si])/result[ai, vi, si]+0.000001)])
